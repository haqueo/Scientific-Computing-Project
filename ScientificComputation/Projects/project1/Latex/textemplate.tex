...
Thus the i'th element of this array gives [node i, maximum number of cars of node i over the 200 iterations ]. The code to print and run is below
...
Thus, we have a matrix where indices (corresponding to edges) are \textit{True} if they are traversed by some cars ($ > 0$) in the 200 minute process.
\leavevmode
\newline
