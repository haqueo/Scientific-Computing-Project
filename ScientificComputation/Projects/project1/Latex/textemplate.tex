...
\begin{figure}[h]
\caption{The top 8 nodes which increased the most in peak value, followed by (next line) the top 8 nodes that decreased the most in peak value, all with their peak values.}
\centering
\includegraphics[width=1.1\textwidth]{top8}
\end{figure}

These elements represents the node number and the difference in maximum load from before to when node 30 is congested.
\end{enumerate}
\end{document}
